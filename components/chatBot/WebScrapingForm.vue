...
const handleAddEditBot = handleSubmit(async (values) => {
  ...
  try {
    const parsedData = cleanAndParseJson(arrrayDetail.value);
    scrapData.scrapedData = parsedData;
    const urls = values.url ? [values.url] : []; // Ensure it's an array

    const bot = await $fetch("http://148.113.16.40:5010/scrape", {
      method: "PUT",
      headers: {
