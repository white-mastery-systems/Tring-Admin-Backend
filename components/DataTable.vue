  ...
  <div class="space-y-4">
    <div class="rounded-lg border">
      <UiTable class="text-left text-gray-500">
        <UiTableHeader class="bg-gray-50 text-xs uppercase">
          <UiTableRow v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
            <UiTableHead v-for="header in headerGroup.headers" :key="header.id"
              class="text-nowrap px-6 py-2 text-md font-extrabold text-gray-700" scope="col"
        ...
        <UiTableBody>
          <template class="bg-black-400" v-if="table.getRowModel().rows?.length">
            <UiTableRow v-for="row in table.getRowModel().rows" :key="row.id"
              class="cursor-pointer border-b p-1 even:bg-white" @click="$emit('row-click', row)"
              :data-state="row.getIsSelected() && 'selected'">
              <UiTableCell v-for="cell in row.getVisibleCells()" :key="cell.id" class="px-6 py-4 text-md font-semibold">
        ...
        <UiTableFooter v-if="footer">
          <UiTableRow v-for="footerGroup in table.getFooterGroups()" :key="footerGroup.id">
            <UiTableHead v-for="footer_h in footerGroup.headers" :key="footer_h.id"
              class="px-6 mx-6 font-bold lg:text-lg" :colspan="footer_h.colSpan">
    ...
    <div v-if="table.getFilteredRowModel().rows.length > pageSize"
      ...
      <div class="flex space-x-4">
        ...
        <UiButton size="icon" :disabled="!table.getCanPreviousPage()" @click="table.previousPage()">
        ...
        <UiButton size="icon" @click="table.setPageIndex(table.getPageCount() - 1)">
