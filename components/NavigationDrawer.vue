<template>
  <div class="flex w-[250px] flex-col items-center justify-center gap-5 pt-0">
    ...
    <template v-for="{ name, icon, path, children } in navigationModules" :key="path">
      <UiAccordion v-if="children?.length > 0" type="single" class="w-[90%]" v-model="openAccordions[path]" collapsible>
        <UiAccordionItem :value="path" class="shadow-md border-0 bg-white">
          <div class="field_shadow flex cursor-pointer items-center gap-3 px-[16px] font-medium" :class="[
            route.path?.includes(path)
              ...
              : '',
            (openAccordions[path] === path) ? 'rounded-t-[10px]' : 'rounded-[10px]'
          ]">
          ...
          <div class="rounded-b-lg">
            <UiAccordionContent v-for="(item, index) in children" :key="item.path"
              ...
              :class="[route.path?.includes(item.path) && 'text-[#424bd1]', (index === children.length - 1) ? 'rounded-b-lg' : '']">
