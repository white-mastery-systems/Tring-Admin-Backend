<template>
  <div class="field_shadow w-full overflow-hidden rounded-lg" style="height: 50vh">
    ...
    <div class="h-[calc(100%-70px)] overflow-y-scroll bg-[#e5ddd5]" style="background-image: url(../../whatsapp.png)">
      <div class="h-[calc(100%-70px)] max-w-[70%]">
        <div
          class="field_shadow mx-2 mt-2.5 flex min-h-[40px] max-h-[300px] flex-col gap-2 rounded-r-xl rounded-bl-xl bg-[#ffffff] p-5 w-[40%]">
          <div v-if="templateStore?.values?.header === 'image'">
            <img v-if="templateStore?.values?.headerFile?.url" :src="templateStore?.values?.headerFile?.url" alt=""
              style="height: 250px; width: 100%;" />
          </div>
          <div v-else-if="templateStore?.values?.header === 'video'">
            <video v-if="templateStore?.values?.headerFile?.url" :src="templateStore?.values?.headerFile?.url" controls
              width="100%">
              Your browser does not support the video tag.
            </video>
          </div>
          <div v-else-if="templateStore?.values?.header === 'document'"
            class="flex min-h-[10vh] min-w-[10vw] items-center justify-center bg-gray-100">
            <div class="shadow-md flex h-[30px] w-64 items-center justify-center rounded-lg p-2">
              <ScrollText v-if="templateStore?.values?.headerFile?.url" class="h-10 w-10 text-gray-500"></ScrollText>
              <StickyNote v-else class="h-10 w-10 text-gray-500" />
              <!-- Adjust size if needed -->
            </div>
          </div>
          <div v-else-if="templateStore?.values?.header === 'Location'">
          ...
          <div class="flex flex-col">
            <div class="flex flex-wrap items-center gap-2 ">
              <!-- max-w-[250px] -->
              <div v-if="templateStore?.values?.body" class="flex items-center w-full overflow-scroll ">
