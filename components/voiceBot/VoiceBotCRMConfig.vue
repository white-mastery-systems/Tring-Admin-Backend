<template>
  <div class="pb-7">
    ...
    <div class="pb-0">
      <DataTable :columns="columns" :data="integrations" :page-size="8" :is-loading="loading" :height="20"
...
// Watch for data changes and transform for display
      ...
      item.integration?.crm === "zoho-bigin"
        ...
        : item.integration?.crm === "zoho-crm"
          ...
          : item.integration?.crm === "sales-handy"
            ...
            : item.integration?.crm === "hubspot"
              ? `${item?.metadata?.stage}`
              : item.integration?.crm === "reserve-go"
                ? `${item?.metadata?.restaurantId}`
