  ...
  <div class="py-0 sm:px-[10px] md:px-[25px] lg:px-[25px]">
    ...
    <div class="p-[20px] max-h-[80vh] overflow-y-scroll px-4 pb-4 field_shadow">
      <div class="flex items-center justify-between gap-2 pb-4">
        ...
        <div class="flex items-center pr-[10px] gap-3">
          <span class="font-bold text-[9px] sm:text-[10px] md:text-[12px] lg:text-[16px] xl:text-[16px]">
            Total Chats:
            <span style="color: rgba(66, 75, 209, 1)">{{
              analyticsData?.chats
            }}</span>
          </span>
          <span class="font-bold text-[9px] lg:text-[16px] md:text-[12px]">
            Total Leads:
            <span style="color: rgba(66, 75, 209, 1)">{{
              analyticsData?.leads
            }}</span>
      ...
      <DataTable :data="leads" :is-loading="isDataLoading" :columns="columns" :page-size="8" @row-click="(row: any) => {
        console.log({ row });
