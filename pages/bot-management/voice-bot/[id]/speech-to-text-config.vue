    ...
    <form @submit="createEditSpeecToTextConfig">
      <div class="grid w-full grid-cols-2 gap-2">
        ...
        <SelectField
          ...
          :options="adaptations"
          label="adaptation"
          placeholder="Select adaptation"
          helperText="Select your adaptation."
        >
        </SelectField>
        <SelectField
          v-if="values.provider === 'google' || values.provider === 'deepgram'"
          name="model"
          :options="models"
          label="model"
      ...
      <div class="mb-2">
          ...
          <fieldset v-for="(field, idx) in fields" :key="field.key">
            <div class="mt-2 flex items-end gap-2">
              <TextField
                :label="`keyword ${idx + 1}`"
                :id="`value_${idx}`"
                :name="`keywords[${idx}].value`"
                required
                placeholder="Enter keyword"
              />
              <TextField
                :label="`boost value ${idx + 1}`"
                :id="`value_${idx}`"
                :name="`keywords[${idx}].boostValue`"
                required
        ...
        <FieldArray
          v-if="values.provider === 'google' || values.provider === 'azure'"
          name="phraseSets"
          v-slot="{ fields, push, remove }"
        >
          <fieldset v-for="(field, idx) in fields" :key="field.key">
            <div class="mt-2 flex items-end gap-2">
              <TextField
                :label="`phrase ${values.provider === 'azure' ? 'list' : 'set'} ${idx + 1}`"
                :id="`value_${idx}`"
                :name="`phraseSets[${idx}].value`"
                required
...
<script setup lang="ts">
  ...
  watch(botData, (newValue) => {
    ...
    setFieldValue(
      "amplificationFactor",
      botData.value.speechToTextConfig.amplificationFactor,
    );
    setFieldValue("endpointing", botData.value.speechToTextConfig.endpointing);
    setFieldValue(
      "utteranceEndMs",
      botData.value.speechToTextConfig.utteranceEndMs,
    );
  });
