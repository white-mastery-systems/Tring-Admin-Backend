...
<template>
  ...
  { label: 'Bot Configuration', to: `/bot-management/chat-bot/${botDetails.id}/config` },
    ...
    <div class="mx-5">
      <UiForm v-slot="{ values, errors }" :validation-schema="formSchema" :keep-values="true" :validate-on-mount="false"
        :initial-values="defaultFormValues" class="space-y-2" @submit="handleUpdateBotConfig">
        ...
        <div class="flex gap-4">
            ...
            <UiFormItem v-auto-animate="animationProps" class="w-full">
              ...
              <UiFormControl>
                <UiSelect v-bind="componentField">
                  ...
                  <UiSelectContent>
                    <UiSelectItem v-for="({ value, label }, index) in roles" :value="value">{{ value }}
          ...
          <UiFormField v-slot="{ componentField }" name="GOAL">
            <UiFormItem v-auto-animate="animationProps" class="w-full">
              <UiFormLabel>Bot's Goal <UiLabel class="text-lg text-red-500">*</UiLabel>
              </UiFormLabel>
              <UiFormControl>
                <UiSelect v-bind="componentField">
                  <UiSelectTrigger>
                  ...
                  <UiSelectContent>
                    <UiSelectItem v-for="({ value, label }, index) in goals" :value="value">{{ value }}
                      <p class="text-xs italic text-gray-500">{{ label }}</p>
                    </UiSelectItem>
                  </UiSelectContent>
                </UiSelect>
                <UiFormField v-if="componentField.modelValue === 'Other'" v-slot="{ componentField }" name="otherGoal">
                  <UiFormItem v-auto-animate="animationProps" class="w-full">
                    <UiFormControl>
                      <UiInput v-bind="componentField" type="text" />
                    </UiFormControl>
              ...
              <UiFormMessage />
              <span class="text-xs text-gray-500">The bot will be driving the conversation towards this
                goal</span>
            </UiFormItem>
          </UiFormField>
          <!-- <UiFormField v-slot="{ componentField }" name="GOAL">
