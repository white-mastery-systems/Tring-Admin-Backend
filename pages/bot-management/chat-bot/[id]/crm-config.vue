<template>
  ...
  { label: 'CRM Configuration', to: `/bot-management/chat-bot/${botDetails.id}/crm-config` },
  ]" :disableSelector="true">
    <template #actionButtons v-if="integrations.length === 0">
      <UiButton @click="crmConfigModalState.open = true" variant="outline" color="primary">
        Link CRM
      </UiButton>
    </template>
    {{ console.log({ integrations }) }}
    <DataTable :columns="columns" :data="integrations" :page-size="8" :is-loading="false" />
    <ConfigurationModal v-model="crmConfigModalState" @success="handleSuccess()" />
