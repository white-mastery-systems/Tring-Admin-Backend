...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const getOrgCurrentActivePlan =
      await db.query.orgSubscriptionSchema.findFirst({
        where: and(
    ...
    if (
      getOrgCurrentActivePlan?.planCode !== "chat_free" &&
      getOrgCurrentActivePlan?.planCode !== "voice_free" &&
      getOrgCurrentActivePlan?.status !== "cancelled"
    ) {
      const expiryDate = momentTz(getOrgCurrentActivePlan?.expiryDate)
        .tz(timeZone)
        .toDate();
      ...
      if (currentDate < expiryDate) {
        return sendError(
          ...
          createError({
            statusCode: 400,
            statusMessage:
              "Plan change not allowed. You can only subscribe to a new plan after the current plan expires.",
          }),
        );
      }
    }
