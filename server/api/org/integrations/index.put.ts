...
export default defineEventHandler(async (event) => {
  ...
  const generatedAuthResponse: any = await $fetch(
    `https://accounts.zoho.in/oauth/v2/token?client_id=1000.7ZU032OIFSMR5YX325O4W3BNSQXS1U&grant_type=authorization_code&client_secret=922f18d9e0d820fbebb9d93fee5cc8201e58fbda8c&redirect_uri=http://localhost:3000/settings/${body.crm}&code=${body.metaData.code}`,
    { method: "POST" },
  );
  console.log(
    `https://accounts.zoho.in/oauth/v2/token?client_id=1000.7ZU032OIFSMR5YX325O4W3BNSQXS1U&grant_type=authorization_code&client_secret=se&redirect_uri=http://localhost:3000/settings/${body.crm}&code=${body.metaData.code}`,
  );
  console.log({ generatedAuthResponse: generatedAuthResponse });
  if (!integration) {
    return integration;
  }
