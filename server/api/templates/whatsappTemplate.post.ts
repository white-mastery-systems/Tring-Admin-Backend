...
import { createWhatsappMessageTemplate, deStructureVariables, deStructureVariables2 } from "~/server/utils/template";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { bodyVariables, headerVariables, buttonVariables, headerText, bodyText, footerText } = deStructureVariables2(metadata);
    return { bodyVariables, headerVariables, buttonVariables, headerText, bodyText, footerText }
  } catch(error:any) {
