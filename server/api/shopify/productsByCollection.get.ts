...
export default defineEventHandler(async (event) => {
  try  {
   ...
   logger.info(`Fetching Shopify integration details for ID: ${shopifyIntegrationId}`);
   ...
   if(!query?.collectionId) {
     throw createError({
       statusCode: 400,
       message: "collectionId query parameter is required",
     });
   }
  
   const data: any = await $fetch(`https://${shopifyIntegration.shop}/admin/api/2024-04/collections/${query?.collectionId}/products.json`, {
      method: "GET",
      headers: {
   ...
   return data?.products;
    ...
    console.log({ error })
    logger.error(`get shopify products by collection: error, ${JSON.stringify(error)}`)
