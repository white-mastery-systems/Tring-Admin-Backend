...
export const generateCliqAccessToken = async (body: any) => {
  try {
    const response = await $fetch(`https://accounts.zoho.in/oauth/v2/token?code=${body.metadata.code}&client_id=${cliqClientId}&redirect_uri=${config.redirectionUrl}/${body.crm}&grant_type=authorization_code&client_secret=${cliqClientSecret}`, 
      { method: "POST" }
    )
    return response
  } catch(error) {
