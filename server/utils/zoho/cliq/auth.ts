...
export const generateCliqAccessToken = async (body: any, config: any) => {
  try {
    console.log({ body, config })
    const response = await $fetch(`https://accounts.zoho.in/oauth/v2/token?code=${body.metadata.code}&client_id=${cliqClientId}&redirect_uri=${config.redirectUrl}/${body.crm}&grant_type=authorization_code&client_secret=${cliqClientSecret}`, 
      { method: "POST" }
    )
    return response
  } catch(error) {
